...
def main():
    ...
    parser.add_argument('--fs', default='local', type=str, help='local | nfs | nsml')
    ...
    if args.fs == 'local':
        ...
        import nsml
        args.data_dir = os.path.join(nsml.NSML_NFS_OUTPUT, args.data_dir)
        args.metadata_dir = os.path.join(nsml.NSML_NFS_OUTPUT, args.metadata_dir)
        args.output_dir = os.path.join(nsml.NSML_NFS_OUTPUT, args.output_dir)
        args.load_path = os.path.join(args.metadata_dir, args.load_path)
    else:
        import nsml
        args.data_dir = os.path.join(nsml.DATASET_PATH, '/train/')
    ...
    args.init_checkpoint = os.path.join(args.data_dir, args.init_checkpoint.replace(".bin", "") +
    ...
    if args.load_path is not None:
        model.load_state_dict(torch.load(args.load_path, map_location='cpu')['model'])
    ...
    if args.do_predict:
        ...
        output_prediction_file = os.path.join(args.output_dir, "%s.predictions.json" % os.path.basename(args.predict_file))
        output_nbest_file = os.path.join(args.output_dir, "%s.nbest_predictions.json" % os.path.basename(args.predict_file))
        output_weight = os.path.join(args.output_dir, "pytorch_model_squad_finetuned.bin")
        ...
        torch.save(checkpoint, output_weight)
